...
export let CHAT_HISTORY = new BufferMemory({ returnMessages: true, memoryKey: "chat_history" }) ;
/*
...
const modelFactories: Record<string, ModelFactory> = {
  openai: {
    ...
    initiateModelWithMemory: (options: ModelOptions, context: string) => InvokeModelWithMemoryOpenAI(options, context),
    invokeDirectMessage: (options: ModelOptions, messages: BaseLanguageModelInput) => invokeDirectMessageOpenAI(options, messages),
  },
  ollama: {
    initiateModel: (options: ModelOptions) => initiateModel(options),
    initiateModelWithMemory: (options: ModelOptions, context: string) => InvokeModelWithMemory(options, context),
    invokeDirectMessage: (options: ModelOptions, messages: BaseLanguageModelInput) => invokeDirectMessageOllama(options, messages),
  },
};

type OllamaModel = "mistral:7b-instruct"| "tinyllama" |"llama3"|"mistral"|"mistral-nemo"|"llama3.1"|"llama3.1:8b-instruct-q8_0"|"phi4"; 
...
export async function invokeDirectMessageOpenAI(modelOptions: ModelOptions, messages: BaseLanguageModelInput): Promise<string> {
    let modelOptionsFix:ModelOptions = modelOptions;
  modelOptionsFix.model = "gpt-4.1";
  return new Promise((resolve, reject) => {
    ...
    openAI.invoke(messages).then((result) => {
...
export async function InvokeModelWithMemoryOpenAI(modelOptions: ModelOptions, context: string): Promise<ConversationChain> {
      let modelOptionsFix:ModelOptions = modelOptions;
  modelOptionsFix.model = "gpt-4.1";
  return new Promise(async (resolve, reject) => {
    let openAI = new ChatOpenAI({
