...
import {
    ...
    callQuestionFormulator, domain_obtainer, getNextObjective, objectiveAchiever,
    prepareOutputMessage, QwBrowserTest,
    strategyFormulator,humanSkipInterrupt
} from "./nodes";
...
export const workflow = new StateGraph(GraphState)
  ...
  .addNode("initial_router", async () => { return {finalOutput:null}  })
  ...
  .addConditionalEdges("human_skip_interrupt_question",
    (state: typeof GraphState.State)=>{ 
      if(!state.currentObjective){
        return "initial_router";
      }
