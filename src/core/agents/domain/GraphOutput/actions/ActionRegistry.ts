import { ACTION_TYPE, GraphOutputAction, IMarkElementAction, IPlaySoundAction, IQuestionAction, ISetBrowserTestOutcomeAction } from "../types";
...
ActionRegistry.register(ACTION_TYPE.MARK_ELEMENT_ACTION, (data: GraphOutputAction): MarkElementAction => {
  const typedData = data as IMarkElementAction;
  const instance = new MarkElementAction(
    typedData.check,
    typedData.selector,
  );
  return instance;
...
ActionRegistry.register(ACTION_TYPE.PLAY_SOUND_ACTION, (data: GraphOutputAction): PlaySoundAction => {
  const typedData = data as IPlaySoundAction;
  const instance = new PlaySoundAction(
        typedData.audioFilename
  );
    return instance;
});
...
ActionRegistry.register(ACTION_TYPE.QUESTION_ACTION, (data: GraphOutputAction):QuestionAction => {
  ...
  const instance = new QuestionAction(
    typedData.question
  );
...
ActionRegistry.register(ACTION_TYPE.SET_BROWSER_TEST_OUTCOME_ACTION, (data: GraphOutputAction): SetBrowserTestOutcomeAction => {
