...
export const QwBrowserTest = async (state: typeof GraphState.State) => {
  ...
  const lastMessage = qwTest.getTest().getChatbotResponse();
    ...
    const newContext = await addInformationOnContextImportantObjectives(qwTest, humanMessage, outcome);
...
async function addInformationOnContextImportantObjectives(currentObjective: QWBrowserTest, humanMessage: HumanMessage,outcomeResult: TypeTestOutcome) {
...
function addResultToOutputEvaluation(currentObjective:QWBrowserTest, outcome: TypeTestOutcome,graphOutput?: IQWGraphOutput): IQWGraphOutput {
