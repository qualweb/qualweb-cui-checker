...
import { INPUT_TYPE, TGraphInput } from "../domain/GraphInput";
...
export const humanSkipInterrupt = (state: typeof GraphState.State) => {
  ...
  const updatedObjectives: Record<string, ObjectiveSerialized> = {
    ...
    const graphInput = { _type: INPUT_TYPE.GRAPH_SKIP_OBJECTIVE_INPUT} as TGraphInput;
    return {  graphInput:graphInput  };
  }
};
