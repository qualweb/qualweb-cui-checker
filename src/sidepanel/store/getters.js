...
export const isCuiSpeechTestsEnabled = (state) => state.cuiSpeechTests;
export const getCurrentRule = (state) => {
  let currentRule = state.currentRule;
  if (!!currentRule) {
    let resultNumber = getResultNumber(state);
    state.resultFilter = {
      passed: resultNumber.passed > 0,
      failed: resultNumber.failed > 0,
      warning: resultNumber.warning > 0,
      inapplicable: resultNumber.inapplicable > 0,
    };
