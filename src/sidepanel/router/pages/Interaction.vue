    ...
    <Loading :message="state">
      ...
      <template v-slot:buttons>
        <div class="button-container">
          <ButtonStyled
            @click="skipObjective"
            :disabled="isCanceled || isSkipping"
            label="Skip Rule"
          />
          <ButtonStyled
            :primary="false"
...
import {
  APIError,
  defaultErrorHandler,
  InteractionListenerFailedError,
  InteractionPortConnectionError,
} from '../../../errors/error.handler.sidepanel';
...
export default {
  ...
  async mounted() {
    ...
    const dispatchOnCriticalError = async () => {
      sendActionShowNotification(
        this.getTabId,
        'Selectors will be reset, please detect chatbot again.',
      );
    ...
    this._port.onMessage.addListener((msg) => {
      if (msg.action === ACTION_PORT.READY) {
        ...
        startLLMInteraction(this.getTabId, this.getSelectors, isSpeechTestsEnabled)
          .then((response) => {
            ...
            if (response.status === STATUS.ERROR) {
              ...
              defaultErrorHandler({
                error: response,
                storeDispatchCallback: dispatchOnCriticalError,
                callback: disconnectTimeout,
                router: this.$router,
              });
            }
          })
