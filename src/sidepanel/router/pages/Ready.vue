    ...
    <div class="container">
      ...
      <div class="evaluation-container">
        ...
        <Checkbox
          ...
          v-model="speechTestsValue"
          @toggle:check="updateSpeechTests($event)"
          bgColor="#e15500"
          checkColor="#ffffff"
        />
      </div>
...
const speechTestsValue = ref(false);
const isDropdownOpen = ref(false);

const evaluated = computed(() => store.getters.getEvaluated);
onMounted(() => {
  // check if has selectors detected
  if (!store.getters.selectorsDetected) {
    router.push('/');
  }
});
...
const isMicrophoneAvailable = computed(() => {
  return store.getters.getIsMicrophoneAvailable;
});
...
const updateSpeechTests = async (event) => {
  speechTestsValue.value = event.checked;
  await store.dispatch('setCuiSpeechTests', event.checked);
};
