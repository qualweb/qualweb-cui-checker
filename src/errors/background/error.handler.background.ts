import { BACKGROUND_EXCEPTION_CALLBACK_HANDLERS, BACKGROUND_EXCEPTION_PORTS_HANDLERS, resolveDefaultCallbackBehavior, resolveDefaultPortHandlerBehavior } from "./error.registry.background";
import {ErrorClass} from "..";
import { CallbackMessagingEvent, MessageResponse, PortsOfCommunication } from "../../messaging/message-types";
 ...
 */
export  function processErrorEventCallbackBackground(error: Error, callback:CallbackMessagingEvent ):boolean {
...
export function processErrorEventPortsBackground(error: Error, ports: PortsOfCommunication): boolean {
      const errorName = error.name;
