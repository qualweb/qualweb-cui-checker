import { CONTENT_EXCEPTION_PORT_HANDLERS, CONTENT_EXCEPTION_CALLBACK_HANDLERS, DEFAULT_CONTENT_PORT_ERROR_SET, DEFAULT_CONTENT_ERROR_SET } from "..";
...
import { ERROR_MESSAGES_CONTENT } from "./errors.definitions.content";
  ...
  export function defaultContentErrorBehaviorCallbackHandler(errorName: string): MessageResponse | null {
      let message =null;
  ...
  export function defaultPortsErrorHandler(message: MessageResponse, port: chrome.runtime.Port): boolean {
 ...
 * @returns  True if the content script should exit processing the request false otherwise.
 */
export  function processErrorEventCallbackContent(error: Error, callback:CallbackMessagingEvent ):boolean {
    if(error instanceof CancellationError){
      return false;
    }
    const errorName = error.name;
    ...
    if (errorHandler) {
      ...
      errorHandler = CONTENT_EXCEPTION_CALLBACK_HANDLERS[ERROR_CLASS_NAME_CONTENT.UNKNOWN_CONTENT_ERROR];
      console.log('Unknown error during interaction workflow:', error);
    }
 ...
 * @returns 
 */
function defaultCallbackErrorHandler(message:MessageResponse, callback: CallbackMessagingEvent):boolean {
