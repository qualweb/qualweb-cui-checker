...
export function sendMessageToTab(
  ...
  callbackOnError: (tabId: number, request: any, responseHandler: CallbackMessagingEvent) => void,
): void {
    ...
    .then((injected) => {
      if (injected) {
        chrome.tabs.sendMessage(tabId, request, (responseRetry) => {
          if (chrome.runtime.lastError) {
            processErrorEventCallbackBackground(
              new ContentInjectionError('Failed to inject content scripts.'),
              responseHandler,
            );
          } else {
