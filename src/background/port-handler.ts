...
function handleContentPort(port: chrome.runtime.Port) {
    ...
    port.onMessage.addListener((msg) => {
      ...
      // Handle messages from content script
        ...
        InteractionManager.getInstance().buildLanggraph(msg.settings).then(() => {
        ...
        InteractionManager.getInstance().streamEvents(msg.message,msg.config).then((finalOutput) => {
            console.log("Final output from interaction:", finalOutput);
            if(finalOutput.response){
            PortCommunication.getInstance().sendMessageToContent({ action: ACTION.MESSAGE, data: finalOutput });
            }
