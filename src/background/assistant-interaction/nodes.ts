...
export const QwBrowserTest = async (state: typeof GraphState.State) => {
  ...
  const systemMessage = {
    ...
    content: `
        ...
        *** Rules of Evaluation:
        ${
          currentObjective.test?.conditions?.pass
            ? `passed: "${currentObjective.test.conditions.pass}"\n`
            : ''
        }
        ${
          currentObjective.test?.conditions?.warn
            ? `warning: "${currentObjective.test.conditions.warn}"\n`
            : ''
        }
        ${
          currentObjective.test?.conditions?.fail
            ? `failed: "${currentObjective.test.conditions.fail}"\n`
            : ''
        }
        ${
          currentObjective.test?.conditions?.inapplicable
            ? `inapplicable: "${currentObjective.test.conditions.inapplicable}"\n`
            : ''
        }
        ***
  ...
  if (currentObjective.test?.code === 'QW-CUI-C8') {
    if (outcomeResult === 'passed') {
      ...
      return {
        finalOutput: finalOutputEvaluation,
        currentEvaluationObjective: null,
        importantContext: [resultServices.content],
      };
    }
  }
