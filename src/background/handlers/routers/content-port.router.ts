...
const portContentMessageHandlerBuilder =
  (instancePortCommunication: PortCommunication, instanceInteractionManager: InteractionManager) =>
  (msg: any) => {
    ...
    try {
      ...
      if (handler) {
        handler({
          instanceInteractionManager: instanceInteractionManager,
          instancePortCommunication: instancePortCommunication,
          msg,
        });
