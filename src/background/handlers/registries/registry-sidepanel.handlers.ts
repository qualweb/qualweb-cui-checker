...
export const INTERACTION_PORT_SIDEPANEL_HANDLERS: Record<string, fnHandlerPortSidepanel> = {
  [ACTION_PORT.START_INTERACTION]: (args: IPortHandlersArgs) => {
    args.instanceInteractionManager.buildLanggraph(args.msg.data.settings, args.msg.data.isSpeechTestsEnabled).then(() => {
    
                  args.instancePortCommunication.sendMessageToSidepanel({ action: ACTION_PORT.START_INTERACTION });
                })
                .catch((error) => {
