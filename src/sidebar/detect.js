      ...
      if (tabs[0]) {
        chrome.tabs.sendMessage(
          ...
          (response) => {
            if (chrome.runtime.lastError) {
              console.error(chrome.runtime.lastError);
              chrome.runtime.sendMessage({ type: "restartContentScript", tab:tabs[0].id });
...
async function startCorrectionChatbot(elementName) {
  return sendActionToActiveTab("correctElementSelection", { element: elementName });
}
...
async function endVerificationElement(elementName) {
  return sendActionToActiveTab("endSucessfulVerification", { element: elementName });
}
