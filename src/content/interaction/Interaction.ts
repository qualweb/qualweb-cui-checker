...
export async function interactWithLLM(port:chrome.runtime.Port,voice:boolean): Promise<void> {
  ...
  let documentOwner = chatbotInterface.getOwnerDocument();
  
  let firstMessages: HTMLElement[] = Array.from(
    documentOwner.querySelectorAll(chatbotInterface.getSelectors().messagesSelector),
  );
  // TODO: this can be a point of problem if interaction is initiated with history full of messages
