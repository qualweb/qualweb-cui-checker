...
import { ACTION_TYPE, GraphOutputAction, IMarkElementAction, IPlaySoundAction, IQuestionAction, ISetBrowserTestOutcomeAction } from "../../../core/agents/domain/GraphOutput";
...
ActionHandlerRegistry.register(ACTION_TYPE.MARK_ELEMENT_ACTION, async (data: GraphOutputAction, args: ActionHandlerArgs): Promise<void> => {
 ...
 */
ActionHandlerRegistry.register(ACTION_TYPE.PLAY_SOUND_ACTION, async (data: GraphOutputAction, args: ActionHandlerArgs): Promise<void> => {
    const typedData = data as IPlaySoundAction;
    const chatbotActions = args.interfaceActions;
    const audioUrl = chrome.runtime.getURL(APP_CONFIG.SPEECH_AUDIO_FOLDER + typedData.audioFilename);
    ...
    while (detectCount <= maxDetects) {
      ...
      checkIfAbortedAndThrowCancellationError(args.signal);
        ...
        try {
            ...
            const mutationManager = new MutationInteractionDetect(chatbotActions,new MessagesManager() ,args.signal);
          ...
          markQuestion(chatbotActions.getChabotElements().getOwnerDocument(), inputText,args.interactionCount);
...
ActionHandlerRegistry.register(ACTION_TYPE.QUESTION_ACTION, async (data: GraphOutputAction, args: ActionHandlerArgs): Promise<void> => {
  ...
  // TODO: Torna dificil testar, preciso de depender em abstractions
    ...
    const mutationManager = new MutationInteractionDetect(chatbotActions,new MessagesManager() ,signal);
    //TODO : remove hardcoded timeout
    mutationManager.setup(
      chatbotActions.getChabotElements().getWindowElement(),
      new TimeoutManager<HTMLElement[]>(5000)
    ...
    if(newElements.length === 0) {
        ...
        sendMessageToBackgroundPort(args.backgroundPort, {
         ...
         throw new ErrorClass.ObserverFoundNoMessagesAfterRetryError('Chatbot stopped responding.');
        }
        continue;
     }
    ASSISTANT_INTERACTION_MESSAGES.push(newElements);
    ...
    // Mark question and response
    markQuestion(chatbotActions.getChabotElements().getOwnerDocument(), typedData.question,args.interactionCount);
    markResponses(newElements, args.interactionCount);
...
ActionHandlerRegistry.register(ACTION_TYPE.SET_BROWSER_TEST_OUTCOME_ACTION, async (data: GraphOutputAction, args: ActionHandlerArgs): Promise<void> => {
    checkIfAbortedAndThrowCancellationError(args.signal);
