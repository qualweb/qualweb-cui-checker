...
class InteractionWorkflow {
  ...
  public init() {
    ...
    if (!this.initialMsg || !this.backgroundPort || !this.configContract) {
      throw new ErrorClass.PortInitialConfigurationError(
        'InteractionWorkflow not properly configured before init.',
      );
    }
...
export function sendMessageToBackgroundPort(
  port: chrome.runtime.Port,
  payload: CommunicationContract,
) {
  try {
    port.postMessage(payload);
  } catch {
    throw new ErrorClass.FailedCommunicationWithBackgroundError(
      'Failed to send message to background interaction bidirectional port.',
    );
  }
}
