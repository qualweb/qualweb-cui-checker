...
export function startSelection() {
  ...
  showMessage(
    "Click on a part of the chatbot. You may need to click multiple times to identify the entire chatbot."
  );
}
      ...
      if (chatbotElement) {
        ...
        if (selectedElements.length >= 3) {
          ...
          if (interactiveElement) {
            ...
            showMessage(
              "Couldn't identify chatbot element. Please try clicking on a different part of the chatbot."
            );
          }
        } else {
          showMessage(
            "Couldn't identify chatbot element yet. Please try clicking on a different part of the chatbot."
          );
        }
      }
    }
...
function boundingBoxApproach(element: HTMLElement): HTMLElement | null {
  ...
  while (potentialParent) {
    ...
    if (areaRatio > 2) {
      // Parent is significantly larger
      ...
      for (const child of Array.from(childElements)) {
        if (
          child.matches(
            'input[type="text"], textarea, .messages, .chat-messages, button[type="submit"], .send-button'
          )
        ) {
          chatbotComponents++;
        }
...
export function unsetGreen(element: HTMLElement | HTMLElement[]): void {
  ...
  const overlayElements = documentOverlay.querySelectorAll(
    ".chatbot-element-overlay"
  );
