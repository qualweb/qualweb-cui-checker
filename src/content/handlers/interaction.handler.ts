...
export async function actionStartVoiceInput(data: IChromeRequest): Promise<void> {
  ...
  try {
    ...
    if (!data.request.settings)
      throw new ErrorClass.SettingsNotFoundError('Settings for interaction are missing');
    if (!data.request.settings.apiKey)
      throw new ErrorClass.ApiKeyNotFoundError('API key for interaction is missing');
    if (!data.request.settings.locale)
      throw new ErrorClass.LocaleNotFoundError('Locale for interaction is missing');
...
export async function skipCurrentObjectiveInteraction(data: IChromeRequest): Promise<void> {
  try {
    if (!currentInteractionAbortController) {
      throw new ErrorClass.NoInteractionInProgressError(
        'No ongoing interaction to skip objective from.',
      );
    }
