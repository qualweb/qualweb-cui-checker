...
export async function detectPopupChatbot(): Promise<object> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let inputElements = popupChatbot.querySelectorAll(
        'input[type="text"], input:not([type]), textarea, div[contenteditable="true"]',
      );
      let inputElement = getFirstElementVisibleFromArray(
      ...
      let response = await detectChatBotPageMutation(
        popupChatbot,
        'Ola',
...
export function detectAndGetSelectorsPageChatbot(): Promise<object> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const chatbot = await identifyElementsPageChatbot(
        HTMLCode,
        document,
        inputElement,
        commonNode,
