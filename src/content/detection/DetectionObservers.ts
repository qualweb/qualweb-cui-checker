 ...
 * @returns
 */
export async function detectChatBotPageMutation(
  nodeTarget: Element,
  initialText: string,
  ...
  return new Promise(async (resolve, reject) => {
    const observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        ...
        for (const addedNode of mutation.addedNodes) {
          if (ignoreInput && (ignoreInput.contains(addedNode) || ignoreInput === mutation.target)) {
    ...
    await simulateInput(
      initialText,
      ignoreInput as HTMLInputElement | HTMLTextAreaElement | HTMLDivElement,
    );
  });
}
      ...
      for (const mutation of mutations) {
        ...
        if (mutation.type === 'childList') {
          ...
          for (let i = 0; i < addedNodes.length; i++) {
            ...
            let resolved = false;
            let score = scoringTreeChatbot(node as HTMLElement);
          ...
          if (getComputedStyle(element).display !== 'none' && iframesNested.length > 0) {
            ...
            for (let i = 0; i < iframesNested.length; i++) {
              ...
              waitForIframeLoad(iframesNested[i] as HTMLIFrameElement)
                .then((iframe) => {
                  let iframeContent: Document | null = (iframe as HTMLIFrameElement)
                    .contentDocument;
