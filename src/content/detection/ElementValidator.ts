...
export class ElementValidator {
    ...
    constructor(elementManualSelector: ElementManualSelector, chatbotServices: IChatbotService, 
  ...
  startConfirmation(selectorKey: string): void {
    ...
    if (selectorKey.includes('windowSelector')) {
      ...
      elements.forEach((el) =>
        this.highlightController.highlight(el, this.chatbotServices.chatbotInterface.getWindowElement()),
      );
    }
  }
  ...
  private getElementHandlers(): Record<string, () => HTMLElement | HTMLElement[] | null> {
    return {
      ...
      messagesSelector: () => Array.from(
        this.chatbotServices.chatbotInterface
          .getOwnerDocument()
          .querySelectorAll<HTMLElement>(this.chatbotServices.chatbotInterface.getMessagesSelector())
      ),
    };
  }
    ...
    async correct(selectorKey: string, userMessage: string,signal: AbortSignal): Promise<string> {
      ...
      const newSelectors = await this.chatbotServices.chatbotDetector.initDetection(userMessage,jointSignal);
     
      oldSelector[selectorKey] = newSelectors[selectorKey];
    
      this.chatbotServices.chatbotInterface.updateSelector(selectorKey as keyof typeof oldSelector, newSelectors[selectorKey]);
  ...
  private getElementBySelectorKey(selectorKey: string) {
    const elementGetters: Record<string, () => HTMLElement | HTMLElement[] | null> = this.getElementHandlers();

    const handler = elementGetters[selectorKey];
    if (!handler) throw new Error.InvalidSelectorProvidedError(`Invalid selector key: ${selectorKey}`);
    
    const element = handler();

    if (!element) throw new Error.ElementNotFoundError(`Element for ${selectorKey} not found on the page.`);
    return element;
  }
