...
import {
  ...
  InputNotFoundError,
  WindowNotFoundError,
} from '../../errors/content/errors.class.content';
...
class ChatbotDetector {
   ...
   */
  async waitUntilStable(target: Node, idleTime = 1000, maxWait = 2000): Promise<void> {
  ...
  async initDetection(userMessage: string, signal: AbortSignal): Promise<ChatBotSelectors> {
    ...
    const messageSentElement = await this.detectMessagesSentElement(
      inputElement,
      userMessage,
      signal,
    );
    ...
    await this.detectAssistantResponseElements(
      messageSentElement,
      inputElement,
      dialogElement,
      combinedSignal,
  ...
  private async detectAssistantResponseElements(
    messageSentElement: HTMLElement,
  ...
  private async detectMessagesSentElement(
    inputElement: ChatbotInputElement,
    userMessage: string,
    signal: AbortSignal,
  ) {
  ...
  private async detectUserMessage(
    ...
    this.currentMutationManager = new MutationChatbotDetect(
      actions,
      new MessagesManager(),
      inputElement,
      userMessage,
    ...
    this.currentMutationManager.setup(
      inputElement.ownerDocument.body,
  ...
  private async detectChatbotResponse(
    ...
    this.currentMutationManager = new MutationResponseDetect(
      messageSent,
      inputElement,
      combinedSignal,
    );
