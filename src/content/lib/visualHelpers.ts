...
export function setGreen(element: HTMLElement): void {
  ...
  if (element.tagName === "BUTTON" || element.tagName === "TEXTAREA" || element.tagName === "INPUT" || element.isContentEditable) {
...
export function unsetGreen(element: HTMLElement | HTMLElement[]): void {
  ...
  for(const el of elementArray){
      if(el.tagName === "TEXTAREA" || el.tagName === "BUTTON" || el.tagName === "INPUT" || el.isContentEditable){
        el.style.boxShadow = "";
      }else{
