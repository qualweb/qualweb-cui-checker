...
export  function detectChatbotInputCrossOrigin(): ChatbotInputElement| null {
  ...
  for (const iframe of iframes) {
    ...
    const inputs = [
      ...documentIframe.querySelectorAll(
        'input[type="text"], input:not([type]), textarea, div[contenteditable="true"]',
      ),
    ].filter(isVisible);
...
function isFrameworkClass(cls) {
  return /^(m-|mt-|mb-|ml-|mr-|p-|pt-|pb-|pl-|pr-|text-|bg-|flex|grid|gap-|col-|row-|w-|h-|rounded-|ant-|Mui|chakra-|css-|p-)/.test(
    cls,
  );
}
 ...
 */
export function findAncestralNodeBeforeContaining(
  node: HTMLElement,
  ignoreText: string,
): HTMLElement {
  let current = node;
